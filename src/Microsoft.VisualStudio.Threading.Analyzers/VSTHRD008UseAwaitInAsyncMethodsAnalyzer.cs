namespace Microsoft.VisualStudio.Threading.Analyzers
{
    ...
    public class VSTHRD008UseAwaitInAsyncMethodsAnalyzer : DiagnosticAnalyzer
    {
        public const string Id = "VSTHRD008";

        internal static readonly DiagnosticDescriptor Descriptor = new DiagnosticDescriptor(
            id: Id,
            title: Strings.VSTHRD008_Title,
            messageFormat: Strings.VSTHRD008_MessageFormat,
            category: "Usage",
        ...
        internal static readonly DiagnosticDescriptor DescriptorNoAlternativeMethod = new DiagnosticDescriptor(
            id: Id,
            title: Strings.VSTHRD008_Title,
            messageFormat: Strings.VSTHRD008_MessageFormat_UseAwaitInstead,
            category: "Usage",
            defaultSeverity: DiagnosticSeverity.Warning,
            isEnabledByDefault: true);
        ...
        private class MethodAnalyzer
        {
            ...
            internal void AnalyzeInvocation(SyntaxNodeAnalysisContext context)
            {
                if (IsInTaskReturningMethodOrDelegate(context))
                {
                    ...
                    if (symbolInfo.Symbol != null && !symbolInfo.Symbol.Name.EndsWith(VSTHRD010AsyncSuffixAnalyzer.MandatoryAsyncSuffix) &&
                        !(methodSymbol?.ReturnType?.Name == nameof(Task) && methodSymbol.ReturnType.BelongsToNamespace(Namespaces.SystemThreadingTasks)))
                    {
                        string asyncMethodName = symbolInfo.Symbol.Name + VSTHRD010AsyncSuffixAnalyzer.MandatoryAsyncSuffix;
                        ...
                        if (asyncMethodMatches.Any(m => !m.IsObsolete()))
                        {
                            ...
                            var properties = ImmutableDictionary<string, string>.Empty
                                .Add(VSTHRD008UseAwaitInAsyncMethodsCodeFix.AsyncMethodKeyName, asyncMethodName);
                ...
                if (typeReceiver != null)
                {
                    foreach (var item in problematicMethods)
                    {
                            ...
                            typeReceiver.BelongsToNamespace(item.ContainingTypeNamespace))
                        {
                            ...
                            {
                                properties = properties.Add(VSTHRD008UseAwaitInAsyncMethodsCodeFix.AsyncMethodKeyName, item.AsyncAlternativeMethodName);
                                descriptor = Descriptor;
                                messageArgs.Add(item.AsyncAlternativeMethodName);
                            }
                            else
                            {
                                properties = properties.Add(VSTHRD008UseAwaitInAsyncMethodsCodeFix.AsyncMethodKeyName, string.Empty);
